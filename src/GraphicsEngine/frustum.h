...
struct Frustum {
  ...
  void update(Camera *cam, float aspect, float fovY, float near, float far)
  {
    ...
    this->planes[2] = { pos, glm::cross(frontMultFar - cam->right * halfHSide, cam->up) };
    this->planes[3] = { pos, glm::cross(cam->up, frontMultFar + cam->right * halfHSide) };
    this->planes[4] = { pos, glm::cross(cam->right, frontMultFar - cam->up * halfVSide) };
    this->planes[5] = { pos, glm::cross(frontMultFar + cam->up * halfVSide, cam->right) };
  };


  bool visible(glm::vec3 pos, float radius)
  {
    float dist;
