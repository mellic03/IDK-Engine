...
bool Animation::loadKeyframes(std::string filepath, std::string name, int num_keyframes)
{
  ...
  for (int i=0; i<num_keyframes; i++)
  {
    for (int j=0; j<this->keyframes[i].num_vertices; j++)
    {
      ...
      this->keyframes[i].vertices[j].anim_next_normal = this->keyframes[(i+1)%num_keyframes].vertices[j].normal;
      this->keyframes[i].vertices[j].anim_next_tangent = this->keyframes[(i+1)%num_keyframes].vertices[j].tangent;
    }
