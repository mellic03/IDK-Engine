...
void DrawECS::sphere(GameObject *object, SphereColliderComponent *sphereColliderComponent)
{
  ...
  if (ImGui::CollapsingHeader(label.c_str(), ImGuiTreeNodeFlags_DefaultOpen))
  {
    ...
    EngineUI::dragVec3("Position offset", &object->spherecollider.position_offset, 0.0f, 0.0f, 0.1f, "%0.2f", 0.0f);
  }
}
...
void DrawECS::physics(GameObject *object)
{
  if (ImGui::CollapsingHeader("Physics", ImGuiTreeNodeFlags_DefaultOpen))
  {
    ImGui::Text("BitFlags");
    ImGui::Separator();
    GLuint *physicsFlags = object->getData()->physData()->flags()->bits();
    EngineUI::bitFlagCheckbox("VELOCITY", (GLuint)PhysicsFlag::VELOCITY, physicsFlags);
    EngineUI::bitFlagCheckbox("GRAVITY ", (GLuint)PhysicsFlag::GRAVITY,  physicsFlags);
  }
}
